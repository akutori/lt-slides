# LT Slides

勉強会・LT登壇で使用したスライド集です。

## LT一覧

- [現代の電話交換機周りについて](./LT_現代の電話交換機周りについて(OSSのAsteriskを例にPBXの機能について解説)/README.md)
- [Asteriskのビルド方法について](./Asteriskのビルド方法について/README.md)
- [Asteriskのダイヤルプラン設定編](./Asteriskのダイヤルプラン設定編/README.md)
- [PHP ZipArchiveで文字化けする](./PHP%20ZipArchiveで文字化けする/README.md)
- [Pyinfraについて](./Pyinfraについて/README.md)
- [Faxの「アレ」について](./Faxの「アレ」について/README.md)
- [文字化けの犯人はQRじゃなかった](./文字化けの犯人はQRじゃなかった/README.md)
- [ほぼAIだよりで作成したシステムの課題・反省点](./ほぼAIだよりで作成したシステムの課題・反省点/README.md)
