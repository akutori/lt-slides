...
<!-- これは 自分が実際に遭遇した問題で､備忘録も兼ねてまとめました。 -->
...
<!-- まず ZipArchive の説明を簡単に。PHP 5.2 から使えるクラスで、zip ファイルを開いたり展開したりできます。内部でlibzipというCライブラリを使っているようです。
...
Windowsの機能でzipにしたファイルは **CP932(Shift-jis)** でエンコードされているため、PHPの`ZipArchive`が**文字エンコード未指定の場合UTF-8で解釈**してしまい
文字化けが発生する
...
<!-- 原因はエンコーディングのズレです。
Windows の標準 zip 機能で作成したファイルはファイル名が CP932 つまり Shift-JIS でエンコードされています。
...
これに限らず文字エンコードが直接関係するようなファイルなども同様の問題が発生する可能性がある

~~ある程度の制限か割り切りは必要かもしれない~~

<!-- さらにやっかいなことに、Windows でも zip を作るソフトウェアによってエンコードが変わります。
Windows 標準と Lhaplus は CP932、7zip は UTF-8 です。Lhplusとか今使ってる人います???
なので単純に『CP932 に変換すればいい』ではなく、どのエンコードで作られた zip かを判定する必要があります。 -->

<!-- 実際にzipファイルを利用するお客様がいたとして「このツールでzip作ってください」とガイダンスを出すか､諦めるかの判断が必要です。 -->
...
> ※ 注意
> `mb_check_encoding` のエンコード推定はあくまで推定であり、確実な判定にはならない点に注意してください。
...
これが実際に私が行った解決方法で、多分もっと良い方法があるとは思いますが一旦利確したらうまくいきました｡

これで Windows 標準・Lhaplus・7zip のどれで作った zip でも正しく展開できます。 -->

<!-- 注意してほしいのは mb_check_encoding のエンコード推定はあくまで推定であり、確実な判定にはならない点です。 -->

<!-- 文字化けしているということは別の文字エンコードで解釈可能であると判断されているため｡完全な判定は困難です。 -->
...
<!-- 整理すると4点です。
zip のエンコードはソフトによって変わる、ZipArchive はデフォルト UTF-8 解釈、FL_ENC_RAW で生バイト取得、mb_check_encoding で判定して変換、この流れを押さえておけば同じ問題は回避できます。

あとこれは私見ですが PHPの拡張機能はCのライブラリ使っていることが多いのでPHPがおかしいというよりは直接実装見たほうが早いかもしれません 
